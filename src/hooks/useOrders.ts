...
export const useUpdateOrderStatus = () => {
    return useBasicMutation<UpdateOrderStatusResponse, { orderId: number; status: OrderStatus }>({
        f: async ({ orderId, status }) => {
            const response = await request<UpdateOrderStatusResponse>('PUT', `/orders/${orderId}/status`, { status });
            return response;
        },
    });
};